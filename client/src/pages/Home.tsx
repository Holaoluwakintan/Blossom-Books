...
function mapStoryToBook(story: { id: number; slug: string; title: string; authorName: string; subtitle?: string | null; description: string; category: string; rating?: string | null; reviewCount: number; estimatedMinutes: number; coverUrl?: string | null; chapters?: Array<unknown> }) : Book {
  return { id: story.slug ?? String(story.id), title: story.title, author: story.authorName, eyebrow: story.subtitle ?? story.category, description: story.description, category: story.category, rating: story.rating ?? "0", reviews: story.reviewCount, minutes: story.estimatedMinutes, chapters: story.chapters?.length ?? 1, image: story.coverUrl ?? coverImages.golgotha, accent: "#d87558" };
}
...
export default function Home() {
  ...
  useEffect(() => { if (savedQuery.data) setSavedIds(savedQuery.data.map((row) => String(row.story.id))); }, [savedQuery.data]);
  const [search, setSearch] = useState("");
  const handleSave = (id: string) => {
    const book = books.find((item) => item.id === id);
    const isSaved = savedIds.includes(id);
    if (isAuthenticated && Number.isFinite(Number(id))) { saveStory.mutate({ storyId: Number(id) }); } else setSavedIds((items) => isSaved ? items.filter((item) => item !== id) : [...items, id]);
...
export function Discover() {
  ...
  const databaseBooks: Book[] = (databaseStories.data ?? []).map((story) => ({ id: String(story.id), title: story.title, author: story.authorName, eyebrow: story.subtitle ?? story.category, description: story.description, category: story.category, rating: story.rating ?? "0", reviews: story.reviewCount, minutes: story.estimatedMinutes, chapters: 1, image: story.coverUrl ?? coverImages.golgotha, accent: "#d87558" }));
  const visibleBooks = databaseStories.data?.length ? databaseBooks : filteredBooks;
  const handleSave = (id: string) => { if (isAuthenticated && Number.isFinite(Number(id))) saveStory.mutate({ storyId: Number(id) }); else setSavedIds((items) => items.includes(id) ? items.filter((item) => item !== id) : [...items, id]); };
...
export function Reader() {
  ...
  const book = databaseStory.data ? mapStoryToBook(databaseStory.data) : books.find((item) => item.id === params?.id) ?? books[0];
  const { isAuthenticated } = useAuth();
  const storyId = databaseStory.data?.id ?? Number(params?.id);
  const totalChapters = databaseStory.data?.chapters?.length ?? book.chapters;
  const [chapter, setChapter] = useState(0);
  const [fontSize, setFontSize] = useState("medium");
  const [showSettings, setShowSettings] = useState(false);
  const progressQuery = trpc.reader.progress.useQuery({ storyId }, { enabled: isAuthenticated && Number.isFinite(storyId) });
