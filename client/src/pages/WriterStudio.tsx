...
export default function WriterStudio() {
  ...
  return <div className="writer-studio">
    <header className="studio-header"><Link href="/community" className="back-link"><ArrowLeft size={16} /> Community</Link><div className="studio-brand"><span className="brand-mark"><span /></span><strong>Writer Studio</strong></div><div className="studio-header-actions"><Link href="/studio/books" className="button button-outline">Create a book</Link><span className="studio-save-state">{save.isPending ? "Saving…" : lastSaved ?? "All changes saved"}</span><button className="button button-outline" onClick={() => setPreview((value) => !value)}><Eye size={15} /> {preview ? "Edit" : "Preview"}</button><button className="button button-primary" disabled={publish.isPending || !draftId} onClick={() => draftId && publish.mutate({ type: kind, id: draftId })}><Send size={15} /> Publish</button></div></header>
